...
[https://github.com/chloeyean/Yean_Chloe_201_A06](https://github.com/chloeyean/Yean_Chloe_201_A06)
...
```{r}
pts_index <- player_info[[2]][[4]]
pts_index

pts_names <- player_info$LJ$points
pts_names
```
...
round_mat <- apply(mat_rand, MARGIN = 1:2, FUN = my_round)
...
```{r}
annual_temps <- apply(CO[,3:14], 1, median)

max_index <- which.max(annual_temps)
min_index <- which.min(annual_temps)

temp_max <- annual_temps[max_index]
temp_min <- annual_temps[min_index]

print(paste0('The maximum median annual temperature of ', temp_max, ' occurred in ', CO$year[max_index], ' and the minimum median annual temperature of ', temp_min, '  occurred in ', CO$year[min_index], '.'))
```
